...
case of ``ExceptionGroup("three")`` in the example above.  If the condition does
not hold for any of the exceptions in the group, ``subgroup`` returns ``None``
rather than an empty ``ExceptionGroup``.  The original ``eg``
